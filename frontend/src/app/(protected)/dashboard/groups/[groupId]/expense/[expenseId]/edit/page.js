...
export default function EditExpenseForm() {
  ...
  const fetchExpense = async () => {
    try {
      ...
      const res = await apiFetch(`/group/${groupId}/expense`);
      const expense = res.expenses?.find((e) => e._id === expenseId);

      if (!expense) {
        setSubmitError("Expense not found.");
        return;
      }

      const mappedParticipants = (expense.splits ?? []).map((s) => ({
        userId: typeof s.user === "object" ? s.user._id : s.user,
        value: expense.splitType === "equal" ? "" : s.amount,
      }));
      ...
      if (error.statusCode === 404) {
        setSubmitError("Expense or group not found.");
      } else {
        setSubmitError("Failed to load expense.");
      }
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.statusCode === 403) {
        setSubmitError("You are not authorized to edit this expense.");
        return;
      }
      if (error.statusCode === 404) {
        setSubmitError("Expense or group not found. It may have been deleted.");
        return;
      }
      if (!navigator.onLine) {
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-[#8fa6d2]">
                Update the expense details for this group.
              </p>
            </div>
          ...
          {isLoading ? (
            <div className="space-y-4 p-4 sm:p-6">
              {Array.from({ length: 5 }).map((_, i) => (
                <div
                  key={i}
                  className="h-12 animate-pulse rounded-xl border border-[#17345f] bg-[#071a42]"
                />
              ))}
            </div>
          ) : (
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    {membersLoading
                      ? Array.from({ length: 4 }).map((_, i) => (
                          <div
                            key={i}
                            className="h-20 animate-pulse rounded-xl border border-[#17345f] bg-[#071a42]"
                          />
                        ))
                      : members.map((member) => {
                          ...
                          return (
                            <div key={member._id} className="flex flex-col gap-1.5">
                                ...
                                <div className="min-w-0 flex-1">
                                  <div
                                    className={`truncate text-sm font-semibold ${
                                      isSelected ? "text-[#dbe7ff]" : "text-[#c8d7f0]"
                                    }`}
                                  >
                                    {member.name}
                                  </div>
                                  ...
                                  {isSelected &&
                                    ...
                                    totalAmount > 0 && (
                                      <div className="mt-1 text-xs text-[#8ff0ff]">
                                        ₹
                                        {(
                                          totalAmount / formData.participants.length
                                        ).toFixed(2)}
                                      </div>
                                    )}
                                </div>
                                {isSelected && (
                                  <Check size={18} className="text-[#00CDFF]" />
                                )}
                              </button>
                              ...
                              {isSelected && formData.splitType !== "equal" && (
                                <div className="relative">
                                  {formData.splitType === "manual" && (
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#7f97c3] text-sm">
                                      ₹
                                    </span>
                                  )}
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleParticipantValue(member._id, e.target.value)
                                    }
                                    placeholder={
                                  ...
                                  />
                                  {formData.splitType === "percentage" && (
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[#7f97c3] text-sm">
                                      %
                                    </span>
                                  )}
                                </div>
                      ...
                      {formData.splitType !== "equal" && (
                        <p
                          className={`text-xs font-semibold transition-colors ${
                            isSumValid ? "text-[#8ff0ff]" : "text-[#ff9bb7]"
                          }`}
                        >
                          {formData.splitType === "manual"
